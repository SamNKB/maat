...
Estrutura e taxonomia implementadas; inferência, backends e análises ainda são stubs (`NotImplementedError`). **Binária e discreta consolidadas** (§2.5 e §3.1 do fluxo, cada uma com subpágina em docs/tipos/). Fila de discussão: **contínua** é a próxima, depois cauda longa, ordinal, textual, temporal. Depois: implementar `core/inference.py` + `PandasBackend` e rodar o benchmark nos 40 datasets. Ideia aceita em avaliação: rodar ydata-profiling (concorrente mais próximo) nos 40 como baseline de comparação.
